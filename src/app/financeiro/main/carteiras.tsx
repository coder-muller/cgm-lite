import { Button } from "@/components/ui/button";
import { Dialog, DialogClose, DialogFooter, DialogHeader, DialogTitle, DialogContent } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import axios, { AxiosResponse } from "axios";
import { PlusCircle, Trash2Icon } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";

interface Carteira {
    id: number;
    chave: string;
    nome: string;
    usuario: number;
    createdAt: Date;
    updatedAt: Date;
}

export default function Carteiras() {

    const [carteiras, setCarteiras] = useState<Carteira[]>([]);
    const [carteira, setCarteira] = useState<string>("");

    const [isOpen, setIsOpen] = useState(false);

    const loadCarteiras = async () => {
        try {
            const response: AxiosResponse = await axios.get('/api/carteiras?chave=' + localStorage.getItem("chave"));
            if (response.status === 200) {
                setCarteiras(response.data);
            } else {
                toast.warning("Ops!", { description: response.data.message });
            }
        } catch (error: unknown) {
            if (axios.isAxiosError(error) && error.response) {
                toast.error("Erro!", { description: error.response?.data?.message || "Erro desconhecido" });
            } else {
                const err = error as Error;
                toast.error("Erro!", { description: err.message });
            }
        }
    };

    useEffect(() => {
        loadCarteiras();
    }, []);

    const addCarteira = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const response: AxiosResponse = await axios.post('/api/carteiras', {
                chave: localStorage.getItem("chave"),
                nome: carteira,
            });
            if (response.status === 200) {
                toast.success("Sucesso!", { description: "Carteira adicionada com sucesso!" });
                setCarteira("");
                loadCarteiras();
                setIsOpen(false);
            } else {
                toast.warning("Ops!", { description: response.data.message });
            }
        } catch (error: unknown) {
            if (axios.isAxiosError(error) && error.response) {
                toast.error("Erro!", { description: error.response?.data?.message || "Erro desconhecido" });
            } else {
                const err = error as Error;
                toast.error("Erro!", { description: err.message });
            }
        }
    };

    const deleteCarteira = async (id: number): Promise<void> => {
        try {
            const response: AxiosResponse = await axios.delete('/api/carteiras?id=' + id);
            if (response.status === 200) {
                loadCarteiras();
            } else {
                toast.warning("Ops!", { description: response.data.message });
            }
        } catch (error: unknown) {
            if (axios.isAxiosError(error) && error.response) {
                toast.error("Erro!", { description: error.response?.data?.message });
                console.log(error);
            } else {
                const err = error as Error;
                toast.error("Erro!", { description: err.message });
            }
        }
    };

    return (
        <div className="w-11/12 m-auto flex flex-col items-center justify-start gap-2   ">
            <div className="w-full flex items-center justify-end">
                <Button onClick={() => setIsOpen(true)}>
                    <PlusCircle />
                    Adicionar Carteira
                </Button>
            </div>
            <div className="w-full rounded-md border">
                <Table>
                    <TableHeader>
                        <TableHead>ID</TableHead>
                        <TableHead>Nome</TableHead>
                        <TableHead></TableHead>
                    </TableHeader>
                    <TableBody>
                        {carteiras.map((carteira) => (
                            <TableRow key={carteira.id}>
                                <TableCell>{carteira.id}</TableCell>
                                <TableCell>{carteira.nome}</TableCell>
                                <TableCell><Trash2Icon className="w-4 h-4 cursor-pointer" onClick={() => toast.warning("Atenção!", {
                                    description: "A carteira será excluída permanentemente!",
                                    action: {
                                        label: "Excluir",
                                        onClick: async () => toast.promise( deleteCarteira(carteira.id), {
                                            loading: "Excluindo...",
                                            success: "Carteira excluída com sucesso!",
                                            error: "Ops! Algo deu errado...",
                                        })
                                    }
                                })} /></TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
            </div>
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Carteira</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={addCarteira}>
                        <div>
                            <Label>Nome da Carteira</Label>
                            <Input type="text" placeholder="Nome da Carteira" required value={carteira} onChange={(e) => setCarteira(e.target.value)} />
                        </div>
                        <DialogFooter className="mt-3">
                            <DialogClose asChild>
                                <Button type="reset" variant={"secondary"}>Cancelar</Button>
                            </DialogClose>
                            <Button type="submit">Salvar</Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            </Dialog>
        </div>
    )
}